...
def get_prompt(input, vector_database, closest=None):
    ...
    input_embedding = egnhackaton.text_processing.get_embedding_from_input(
        strip_input(input)
    )

    # From vector find closest n vectors (Alessandro)
    closest_vectors_index = closest_vectors_indexes(
        input_embedding, vector_database, closest=closest
    )
